 ...
 * Only one media events implementation can be associated with the ad session and any attempt to create multiple instances will result in an
 * error.
 */
@interface OMIDSmaatoMediaEvents : NSObject
 ...
 * @return A new media events instance. Returns nil if the supplied ad session is nil or if a media events instance has already been
 * registered with the ad session or if a media events instance has been created after the ad session has started.
 * @see OMIDAdSession
 */
- (nullable instancetype)initWithAdSession:(nonnull OMIDSmaatoAdSession *)session error:(NSError *_Nullable *_Nullable)error;
 ...
 */
- (void)adUserInteractionWithType:(OMIDInteractionType)interactionType NS_SWIFT_NAME(adUserInteraction(withType:));
