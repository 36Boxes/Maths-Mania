 ...
 * Only one ad events implementation can be associated with the ad session and any attempt to create multiple instances will result in an
 * error.
 */
 ...
 * @return A new ad events instance associated with the supplied ad session. Returns nil if the supplied ad session is nil or if an ad
 * events instance has already been registered with the ad session.
 */
 ...
 */
- (BOOL)loadedWithVastProperties:(OMIDSmaatoVASTProperties *_Nonnull)vastProperties error:(NSError *_Nullable *_Nullable)error;
@end
